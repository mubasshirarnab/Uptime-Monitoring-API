...
worker.performCheck = (originalCheckData) => {
    //This is where the check will be performed
    ...
    const _moduleToUse = originalCheckData.protocol === 'http' ? require('http') : require('https')
...
worker.processCheckOutcome = (originalCheckData, status) => {
    ...
    const state = originalCheckData.successCodes.indexOf(status) > -1 ? 'up' : 'down'
